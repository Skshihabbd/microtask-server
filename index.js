  ...
  try {
    ...
    app.get("/userworkerhomes", async (req, res) => {
      ...
      const users = await userData.findOne({
        $and: [{ email: req.query?.email }, { role: "worker" }],
      });
    ...
    app.get("/userworkerhomessubmitpay", async (req, res) => {
      ...
      const users = await submissionCollection
        .find({
          $and: [{ worker_email: req.query.email }, { status: "approved" }],
        })
        .toArray();
    ...
    app.get("/managetasksall", async (req, res) => {
    ...
    // patch a file
    app.patch('/infoupdate', async (req, res) => {
      ...
      // Build the query object to find the user by email
      const query = { email: email };
  console.log(query)
      ...
      const document = { $set: { name: userName, image: userImage } };
  console.log(document)
      try {
          // Perform the update and return the updated document
          const result = await userData.findOneAndUpdate(query, document, { returnDocument: "after" });
  
          if (!result) {
              return res.status(404).send({ error: "User not found" });
          }
  console.log(result)
          res.send(result); // Send back the updated document
      } catch (error) {
    ...
    app.get("/taskcreatorall", async (req, res) => {
    ...
    app.get("/taskcreatorhomesubmit", async (req, res) => {
      ...
      const result = await submissionCollection
        .find({
          $and: [{ creator_email: req.query?.email }, { status: "pending" }],
        })
        .toArray();
      res.send(result);
    });
